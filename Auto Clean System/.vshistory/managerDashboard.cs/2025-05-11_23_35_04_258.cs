    ...
    public partial class managerDashboard : Form {
        ...
        void insertOrderDetail(OrderPage order) {
            try {
                ...
                this.ordersDetailsTableAdapter.insertOrderDetail(order.ServiceName, order.cost);
        ...
        void updateRevenue(int idx, string value) {
            if (idx == -1) {
                object result = this.ordersDetailsTableAdapter.getTotalRevenue();
                long total = Convert.ToInt64(result);
                txtRevenue.Text = total.ToString();
            } else {
                object result = this.ordersDetailsTableAdapter.getTotalRevenueService(value);
            }
