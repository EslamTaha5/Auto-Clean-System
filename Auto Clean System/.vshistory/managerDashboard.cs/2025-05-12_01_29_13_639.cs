    ...
    public partial class managerDashboard : Form {
        ...
        void initReports() {
            ...
            try {
                ...
                // Bind the ComboBox
                reportsChart.ChartAreas[0].AxisX.Interval = 1;
                reportsChart.ChartAreas[0].AxisX.LabelStyle.Angle = -45; // Rotate labels for readability
                reportsChart.Legends.Add("Legend1");
        ...
        void updateRevenue(int idx, string value) {
            ...
            try {
                if (idx == -1) {
                    // Get total revenue for all services
                    object result = ordersDetailsTableAdapter.getTotalRevenue(dateFrom.Date, dateTo.Date);
                    if (result != null && result != DBNull.Value)
                        total = Convert.ToInt64(result);
                } else {
                    // Get revenue for a specific service
                    object result = ordersDetailsTableAdapter.getTotalRevenueService(dateFrom, dateTo, value);
                    if (result != null && result != DBNull.Value)
                        total = Convert.ToInt64(result);
                }

                txtRevenue.Text = total.ToString("C"); // Format as currency
                updateChart(); // Refresh the chart
            } catch (Exception ex) {
                MessageBox.Show($"Error updating revenue: {ex.Message}");
            }
        }
        ...
        void updateChart() {
            try {
                ...
                // Clear existing data
                reportsChart.Series.Clear();

                // Fetch data from the database
                DataTable chartData = ordersDetailsTableAdapter.GetDataBy3(dateFrom, dateTo);

                // Create a new chart series
                Series series = new Series("Service Revenue");
                series.ChartType = SeriesChartType.Column; // Choose chart type: Column, Bar, Pie, etc.
                ...
                foreach (DataRow row in chartData.Rows) {
                    ...
                    series.Points.AddXY(serviceName, totalRevenue);
                }

                // Add series to the chart
                reportsChart.Series.Add(series);

                // Customize chart appearance
                reportsChart.ChartAreas[0].AxisX.Title = "Service Name";
                reportsChart.ChartAreas[0].AxisY.Title = "Revenue ($)";
                reportsChart.Titles.Clear();
                reportsChart.Titles.Add("Service Revenue Report");
            } catch (Exception ex) {
