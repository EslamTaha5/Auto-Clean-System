...
namespace Auto_Clean_System {
    public partial class managerDashboard : Form {
        ...
        void insertOrderDetail(OrderPage order) {
            this.ordersTableAdapter.insertOrder(order.customerID, order.employeeID, order.CarName, order.cost, order.discount, order.totalCost);
            this.fKOrderStaffBindingSource.EndEdit();
            this.tableAdapterManager.UpdateAll(autoCleanDatabaseDataSet);
        }
        void insertOrder(OrderPage order) {
            insertCustomer(order);
            object result = this.customerTableAdapter.FindCustomerID(order.customerName, order.customerID, order.CarName);
            Console.WriteLine($"Insert ORDER :  {result == null}");
            if (result != null && result != DBNull.Value) {
